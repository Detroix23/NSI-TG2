...
def codage_rle(liste_octets):
    ...
    Le compte peut dépasser 255. Or un byte, octet, vaut au maximum 255.
    
    Nous voyons la limitations appliquée dans la fonction `enregistrer_octets`.
    
    Ainsi, nous devons diviser les couple dépassant un compte de 255 en sous-couples.
    Exemple:
       [512, 1] -> [255, 1, 255, 1] 
    '''
    liste_rle = []
    ...
    while i < len(liste_octets):
        ...
        k = 1
        while (
            i+k < len(liste_octets) 
            and liste_octets[i+k] == c
            and k < 255
        ):
            k += 1
...
def decodage_rle(liste_rle: list[int]) -> list[int]:
    '''
    ...
    while index < len(liste_rle):
        count: int = liste_rle[index]
        value: int = liste_rle[index + 1]
...
def test_codage():
    ...
    assert codage_rle([1]) == [1, 1]
    assert codage_rle([]) == []
    assert codage_rle([255, 255, 255, 255, 255]) == [5, 255]
    
    assert decodage_rle([2, 255, 1, 0, 3, 255]) == [255, 255, 0, 255, 255, 255]
    assert decodage_rle([1, 1, 1, 2, 1, 3]) == [1, 2, 3]
    assert decodage_rle([1, 255]) == [255]
    assert decodage_rle([0, 23, 1, 23, 2, 23]) == [23, 23, 23]
...
def encoder_decoder_image(nom_image):
    ...
    width, data_original = charger_image(nom_image)
    data_code = codage_rle(data_original)
    
    print(f"\n## Image: {nom_image}." )
    print(f"{width=}")
    print(f"{data_original[:500]=}")
    print(f"{data_code[:100]=}")
    
    
    enregistrer_octets(nom_image+'.rle', data_code)
    
    data = charger_octets(nom_image+'.rle')
    
    print(f"{data[:50]=}")
    
    enregistrer_image(nom_image+'.dec.png', width, decodage_rle(data))
...
question3 = """
`bac_nsi_256.png`;
`bac_nsi_256.png.dec.png` (Décodé): décalé d'une demi-lettre.
"""

def main() -> None:
    print("\n# BAC 2026. NSI Pratique: sujet 3.\n")
